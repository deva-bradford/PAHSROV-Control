...
dual xresolver(int trigstate, float joystate) { //add function to neutralize the motor before FWB-BWD switching
	float antijoystate;
	int poslimit = 10000;
	int neglimit = -poslimit;

	if (trigstate && ((poslimit > joystate) && (joystate > neglimit))) { //full forward
		antijoystate = joystate = trigstate;
	}
	else if (trigstate && ((poslimit < joystate) || (joystate < neglimit))) { //leaning
		if (joystate < 0) {
			joystate = 0.75f * trigstate;
		}
		else {
			joystate = trigstate;
			antijoystate = 0.75f * trigstate;
		}
	}
	else { //doubleRot
